<template>
  <div v-if="listContent">
    ...
    <table id="table" style="width:100%">
      <tr>
        <th @click="sortBy('firstname')">
          EESNIMI
          <v-icon class="sortIcon">mdi-sort-alphabetical-variant</v-icon>
        </th>
        <th @click="sortBy('surname')">
          PEREKONNANIMI<v-icon class="sortIcon"
            >mdi-sort-alphabetical-variant</v-icon
          >
        </th>
        <th @click="sortBy('sex')">
          SUGU<v-icon class="sortIcon">mdi-sort-alphabetical-variant</v-icon>
        </th>
        <th @click="sortBy('personal_code')">
          SÜNNIKUUPÄEV<v-icon class="sortIcon"
            >mdi-sort-alphabetical-variant</v-icon
          >
        </th>
        <th @click="sortBy('phone')">
          TELEFON<v-icon class="sortIcon">mdi-sort-alphabetical-variant</v-icon>
        </th>
      </tr>
      <tr
        :id="index"
        @click="openDetails(index, person.id)"
        class="rows"
        v-for="(person, index) in paginatedList"
        :key="index"
      >
    ...
    <v-pagination
      ...
      circle
      v-model="page"
      :length="listContent.list.length / 10"
    ></v-pagination>
  </div>
</template>
  ...
  methods: {
    ...
    sortBy(sortedColumn) {
      ...
      if (this.sortOrder === 0) {
        if (typeof sortedColumn === "number") {
          this.listContent.list.sort(
            (a, b) => parseFloat(a[sortedColumn]) - parseFloat(b[sortedColumn])
          );
          this.sortOrder = 1;
          this.chosenSort = sortedColumn;
        } else {
          this.listContent.list.sort((a, b) =>
        ...
        if (typeof sortedColumn === "number") {
          this.listContent.list.sort(
            (a, b) => parseFloat(b[sortedColumn]) - parseFloat(a[sortedColumn])
          );
          this.sortOrder = -1;
          this.chosenSort = sortedColumn;
        } else {
    ...
    openDetails(x, id) {
      ...
      if (this.detailOpen === false) {
        ...
        row.innerHTML =
          "<td colspan='10'> <div id='detailWrapper' style='display: flex; width: 100% !important; height: 165px !important; padding-top: 10px;'> " +
          ` <img style="max-width: 25%; padding-bottom: 30px; margin-right: 25px;" src='${detailContent[0].images[0].medium}'/>` +
